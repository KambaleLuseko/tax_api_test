...
AdminDashboardService.getDataPerUser = async (value) => {
    let groupedData = [];
    let divisionData = [];
    let dataPerUser = await connexion.query(`SELECT SUM(taxations.totalAmount) totalTaxation, (SELECT users.fullname FROM users INNER JOIN accounts ON accounts.user_id=users.id WHERE accounts.uuid=taxations.accountUUID) username,(SELECT SUM(payments.totalAmount) FROM payments WHERE payments.accountUUID=taxations.accountUUID) totalPayment from taxations INNER JOIN accounts on accounts.uuid=taxations.accountUUID INNER JOIN users ON users.id=accounts.user_id GROUP BY taxations.accountUUID, users.fullname`, { type: QueryTypes.SELECT });
    let dataPerDivision = await connexion.query(`SELECT divisions.id,divisions.name divisionName, (SELECT COALESCE(SUM(payments.totalAmount),0) FROM payments WHERE payments.accountUUID=(accounts.uuid)) totalPayment,(SELECT COALESCE(SUM(taxations.totalAmount),0) FROM taxations WHERE taxations.accountUUID=ANY_VALUE(accounts.uuid)) totalTaxation FROM divisions LEFT JOIN users ON users.division_id=divisions.id LEFT JOIN accounts ON accounts.user_id=users.id GROUP BY divisions.id, accounts.uuid`, { type: QueryTypes.SELECT });
